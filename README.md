...
Бот принимает только текстовые сообщения от пользователей из списка разрешённых ID в личном чате, где ID чата совпадает с ID отправителя. Остальные обновления пропускаются без поиска и ответа. Язык RU/EN допускается по распознаваемой грамматической конструкции вопроса, а не по алфавиту или отдельным словам. Если конструкция не распознана, бот предлагает задать вопрос по-русски или по-английски без обращения к базе знаний. Это правило может отклонить корректный вопрос с необычной формулировкой; его можно перефразировать. Лишь для короткого технического обозначения вроде `PDF?` или числа используется язык профиля Telegram; если он неизвестен, бот показывает двуязычную подсказку. История бота хранится в PostgreSQL отдельно от веб-бесед: последние три успешно доставленные пары на каждого отправителя.

Ответ отправляется обычным текстом с названием и местом каждого источника. В `demo` добавляются релевантные выдержки и явная пометка об отсутствии ИИ-ответа. Оригиналы файлов не отправляются. В `real_ai` ошибка провайдера возвращает сообщение об ошибке без перехода в `demo` и без записи пары в историю. Длинный ответ делится на части; курсор обновлений и история сохраняются одной транзакцией только после доставки всех частей. При сбое отправки обновление повторяется. Уже доставленные части при этом могут прийти повторно, поскольку Telegram Bot API не даёт атомарно связать отправку с транзакцией БД. Запускайте только один экземпляр бота с одним токеном и этой базой данных.
