...
Внутренняя база знаний на Next.js, FastAPI и PostgreSQL с расширением pgvector. API поддерживает вход, управление учётными записями и документами, поиск и беседы. Отдельный worker индексирует PDF с текстовым слоем, TXT и Markdown. Интерфейс бесед и Telegram добавляются в следующих этапах.
