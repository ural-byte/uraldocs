...
- «Про что проект?» — ожидается вводная секция актуального `README.md`. Регистр, пробелы и пунктуация этого точного вопроса нормализуются. В `real_ai` вводная секция сразу передаётся модели как единственный источник без embedding-поиска по вопросу. Если актуального `README.md` нет, выполняется обычный поиск текущего режима. В обычном demo-поиске от `README.md` участвует только вводная секция, чтобы инструкции и контрольные вопросы не становились источниками фактов.
...
docker compose run --rm --no-deps -e APP_ORIGIN=http://localhost:3000 -e KB_MODE=demo -e AI_BASE_URL= -e AI_API_KEY= -e AI_EMBEDDING_MODEL= -e AI_CHAT_MODEL= -v "$PWD:/workspace" -w /workspace/api api sh -c 'pip install -q -r requirements-dev.txt && TEST_POSTGRES_URL="$(python -c "from app.config import settings; print(settings.database_url)")" pytest -q'
```

Корень репозитория монтируется целиком, потому что регрессионные тесты читают `README.md` и файлы из `examples`. Миграции создают расширение `vector` в свежей БД. Параметры `-e` действуют только внутри тестового контейнера `api` и не меняют `.env` работающих сервисов. URL PostgreSQL берётся из текущего настроенного окружения; команда передаёт его тестам через обязательный `TEST_POSTGRES_URL`.
