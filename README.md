...
Бот принимает только текстовые сообщения от пользователей из списка разрешённых ID в личном чате, где ID чата совпадает с ID отправителя. Остальные обновления пропускаются без поиска и ответа. Язык вопроса определяется локальной моделью fastText без внешнего вызова: уверенные RU/EN вопросы допускаются к поиску, остальные получают ясную границу до обращения к базе знаний. Для короткого технического обозначения вроде `PDF?` или числа используется язык профиля Telegram; если он неизвестен, бот показывает двуязычную подсказку. Модель, лицензия, атрибуция, проверка целостности и открытая калибровка описаны в [документации модели](api/app/data/README.md). История бота хранится в PostgreSQL отдельно от веб-бесед: последние три успешно доставленные пары на каждого отправителя.
